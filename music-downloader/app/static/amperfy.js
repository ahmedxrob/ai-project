...
async function loadSettings() {
    ...
    try {
        ...
        setValue(
            "set_music_path",
            settings.music_path || "Unknown"
        );

        const storageStatus =
            document.getElementById("set_storage_status");
        ...
        if (storageStatus) {
            try {
                ...
                storageStatus.textContent =
                    storage.available && storage.writable
                        ? "✅ Available and writable"
                        : storage.available
                            ? "⚠️ Available but not writable"
                            : "❌ Storage unavailable";
            } catch (storageError) {
                storageStatus.textContent = "⚠️ Status unavailable";
                console.warn("Storage status:", storageError);
            }
        }
