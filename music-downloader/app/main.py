    ...
    with get_db_connection() as conn:
        conn.execute(
            ...
            (
                ...
                last_updated, final_name, cancel_requested
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
...
def save_settings(data: dict):
    ...
    try:
        settings["max_results"] = max(1, min(int(settings.get("max_results", 20)), 100))
    except Exception:
        settings["max_results"] = 20

    if settings.get("audio_quality") not in {"128K", "192K", "256K", "320K"}:
        settings["audio_quality"] = "320K"

    if settings.get("audio_format") not in {"mp3", "m4a", "opus", "ogg", "wav", "flac"}:
...
def normalize_duplicate_key(value: str) -> str:
    ...
    value = re.sub(r"\b(?:official\s+)?(?:video|audio|music\s+video|lyrics?)\b", " ", value, flags=re.I)
    value = re.sub(r"\b(?:hd|4k|8k|remastered|remaster|audio)\b", " ", value, flags=re.I)
...
def cleanup_task_files(task_id: str):
    patterns = [f"{task_id}.*", f"clean_{task_id}.*"]
...
def _get_all_audio_files_sync():
    ...
    for path in DOWNLOAD_DIR.rglob("*"):
        if not path.is_file() or path.name.startswith(".") or ".covers" in path.parts:
            continue
...
def _is_duplicate_with_artist_sync(title: str, artist: str = ""):
...
async def is_duplicate_with_artist(title: str, artist: str = ""):
    return await asyncio.to_thread(_is_duplicate_with_artist_sync, title, artist)
...
def _resolve_file_sync(filename: str) -> Path:
    ...
    for match in DOWNLOAD_DIR.rglob("*"):
        if not match.is_file() or match.name != target_name:
            continue
        try:
    ...
    try:
        def ping():
            request = urllib.request.Request(
                request_url,
                headers={"User-Agent": "XrobMusic/1.0"},
            )
...
def update_task(task, *, status=None, step=None, percent=None, speed=None, error=None):
    if status is not None:
        task["status"] = status
    if step is not None:
        task["step"] = step
    if percent is not None:
        task["percent"] = max(0, min(float(percent), 100))
    ...
    while True:
        ...
        try:
            ...
            if task.get("cancel_requested") or task.get("status") == "cancelled":
                update_task(task, status="cancelled", step="Cancelled")
            ...
            try:
                update_task(
                    ...
                    percent=max(float(task.get("percent", 0)), 0),
                )
                await notify_task_update(task, force_save=True)
                ...
                embed_thumb = bool(settings.get("embed_thumbnail", True))
                ...
                speed_regex = re.compile(r"at\s+([~0-9.]+[A-Za-z]+/s)")
                ...
                if task.get("cancel_requested"):
                    ...
                    update_task(task, status="cancelled", step="Cancelled", percent=0)
                    await notify_task_update(task, force_save=True)
                    continue
                ...
                if process.returncode != 0:
                    ...
                    if not error_text:
                        error_text = "yt-dlp download failed."

                    await asyncio.to_thread(cleanup_task_files, task_id)
                    update_task(task, status="error", step="Download failed", error=error_text[-1000:])
                    await notify_task_update(task, force_save=True)
                    continue
                ...
                update_task(task, status="processing", step="Cleaning metadata...", percent=95)
                await notify_task_update(task, force_save=True)

                clean_title = clean_filename(task.get("title", "Unknown"))
                clean_artist = clean_filename(task.get("artist", "Unknown Artist"))
                clean_album = clean_filename(task.get("album", ""))
                ...
                if settings.get("organize_by_artist", False):
                    final_dir = DOWNLOAD_DIR / clean_artist
                ...
                if final_path.exists():
                    ...
                    counter = 2
                    while True:
                        candidate = final_dir / f"{stem} ({counter}){ext}"
                ...
                update_task(task, status="completed", step="Ready", percent=100, speed="")
                await notify_task_update(task, force_save=True)
                ...
                ACTIVE_PROCESSES.pop(task_id, None)
                await asyncio.to_thread(cleanup_task_files, task_id)
                update_task(task, status="error", step="Error", error=str(error)[-1000:])
                await notify_task_update(task, force_save=True)
...
async def get_library():
    ...
    files, total_bytes = await asyncio.to_thread(build)
    ...
    def build():
        ...
        for path in files:
            ...
            if len(relative.parts) > 1:
                artists.add(relative.parts[0])

        return len(files), len(artists), total_bytes

    tracks, artists, total_bytes = await asyncio.to_thread(build)
...
async def stream_library_file(filename: str, transcode: bool = Query(False)):
    ...
    media_type = MEDIA_TYPES.get(extension) or mimetypes.guess_type(file_path.name)[0] or "audio/mpeg"
...
async def get_library_cover(filename: str):
    ...
    cache_key = hashlib.md5(
        str(file_path.relative_to(DOWNLOAD_DIR)).encode()
    ).hexdigest() + ".jpg"
...
async def search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"YouTube search failed: {str(error)}",
        )
...
async def preview_audio(url: str = Query(..., min_length=1)):
    ...
    try:
        ...
        if process.returncode != 0:
            error = stderr.decode("utf-8", errors="ignore")
            raise RuntimeError(error[-500:] or "Failed to extract preview stream.")
        ...
        if not stream_url:
            raise RuntimeError("Could not retrieve audio stream.")
...
async def enqueue_download(payload: dict = Body(...)):
    ...
    artist = str(payload.get("artist", "Unknown Artist")).strip()
    ...
    async with DOWNLOAD_LOCK:
        if await is_duplicate_with_artist(title, artist):
            raise HTTPException(
                status_code=409,
                detail="This track already exists in your library.",
            )

        requested_key = normalize_duplicate_key(title)

        for existing in TASKS.values():
            if existing.get("status") not in {"queued", "downloading", "processing"}:
                continue

            existing_key = normalize_duplicate_key(existing.get("title", ""))
...
async def get_tasks():
    ...
    tasks.sort(
        key=lambda task: (
            status_weight.get(task.get("status", ""), 99),
...
async def retry_task(task_id: str):
    ...
    if original.get("status") not in {"error", "cancelled"}:
        raise HTTPException(
            status_code=409,
    ...
    task = {
        ...
        "artist": original.get("artist", "Unknown Artist"),
        "album": original.get("album", ""),
        "url": original.get("url", ""),
        "elementId": original.get("elementId", ""),
        "status": "queued",
        "percent": 0,
        "speed": "",
        "eta": "",
